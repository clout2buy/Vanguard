...
- Completion claimed only through the explicit complete_task control tool; execute_task contracts gate all mutation/execution tools; a blank workspace is never scaffolded without an actionable contract
...
- Repository intelligence: a deterministic filesystem-scanned repository model (languages with support tier, build systems, entry points, test topology, generated dirs, git state, AGENTS.md-style instructions) exposed as the repo_map tool, plus a tiered progressive-verification ladder — a cheap post-edit syntax rung (first-party parse CLIs for deep-tier JS/Python/Go, structural delimiter checks for TypeScript and generic languages) beneath targeted, milestone, sealed, and patch-review rungs
- Long-horizon plan spine: durable engineering contracts (criteria, constraints, non-goals, assumptions, risk, deliverables), a runtime-owned milestone plan whose revisions are journaled and persisted atomically, mutation gated on an initial plan, completion gated on every milestone being proven by referenced evidence or explicitly invalidated, and periodic re-grounding notes that pin the contract and unproven work late in context
