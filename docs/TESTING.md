...
After running `scripts\install-cli.ps1` once, `vanguard` opens the terminal UI from any PowerShell directory. The current directory is locked in automatically and the only setup input is the message. Exit requests are handled locally. Greetings, help, and thanks are real conversational model turns—not a hard-coded reply table—but conversation mode exposes only observation and control tools. They can create a durable conversation session and spend inference, yet they cannot mutate files, run completion verification, or materialize a disposable coding workspace unless the model first accepts an actionable task through an explicit contract. DeepSeek V4 Pro, the expert turn budget, credentials, and verification policy are selected silently. Existing npm, Gradle, pytest, Cargo, Maven, and supported native build contracts are detected automatically. Blank projects use an adaptive verifier that tells the agent—not the user—to establish a deterministic contract before completion.
