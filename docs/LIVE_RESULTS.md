...
- Vanguard passed from the starter on its first completion claim: 62 model decisions, 61 tool calls, 11.4 minutes, 1,908 final source lines, one changed source file, three productive compile/render failures, and no sealed-verification failure. The corrected final oracle accepts this original first-claim source. Its capture metrics were mean luminance `114.2`, luminance deviation `30.2`, `124` quantized colors, and visible lower-screen HUD evidence.
- Claude Code 2.1.204, routed through DeepSeek's Anthropic-compatible endpoint, produced a larger six-file implementation in 132 turns and 39.3 minutes. Its first claim passed the original basic build/system/image-complexity grader but failed the corrected visual oracle: mean luminance `83.3`, deviation `19.7`, `80` quantized colors, and no visible lower-screen HUD evidence.
- Claude received one general recovery message containing only model-readable artifact evidence, not hidden grader source. It repaired camera, lighting, time-of-day, and HUD behavior in 55 more turns and 6.6 minutes. The recovered code passed the corrected sealed grader and protected-file hashes. Final metrics were mean luminance `125.0`, deviation `44.8`, `128` quantized colors, and visible HUD evidence. Claude CLI accounting reported `$10.00` for the first run and `$2.83` for recovery; these are CLI-reported estimates, not independently audited billing.
- The first visual oracle was too weak: it verified dimensions and basic pixel complexity but accepted a visually unreadable artifact. A later byte-identical repeat rule was too strict for small OpenGL/font raster differences and briefly produced a false failure. The final version measures exposure, tonal separation, color breadth, lower-screen HUD evidence, and normalized repeated-pixel stability. Independent Pillow decoding also showed that apparent large black regions in an intermediate app preview were a preview-tool artifact, not framebuffer pixels.
- Additional Vanguard hardening recovered the capture path from `GL_FRONT` after `SwapBuffers` to `GL_BACK` before swapping. That run passed on its second claim, but it is not charged against the original head-to-head because the original first-claim source passes the corrected final oracle.
- Audited conclusion: on this long visual/native task, Vanguard was substantially faster and reached an accepted artifact on its first claim; Claude Code required explicit artifact feedback but also recovered to a valid result. This is strong evidence for Vanguard's long-horizon control loop, not proof of universal Claude Code superiority or parity. Both outputs remain procedural prototypes rather than advanced commercial games.
- Core response: `artifact.inspect_image` now decodes BMP and PNG without a vision model and returns exposure, regional detail/occlusion, dominant colors, HUD-like contrast, a compact luminance map, and normalized pixel comparisons. This gives non-vision DeepSeek actionable visual evidence. The fixed trusted-check tool also ignores provider-added argument noise while retaining its immutable runtime-owned command vector.
