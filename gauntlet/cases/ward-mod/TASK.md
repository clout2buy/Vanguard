...
1. `Claim` represents an inclusive cuboid. Validate nonblank ids/dimensions and non-null owners/positions, reject tabs and line separators that would corrupt persistence, normalize both corners, support dimension-aware `contains` and `overlaps`, and calculate an exact positive `long` volume. Reject coordinate ranges whose volume overflows `long`. Its tab-separated persistence form must round-trip and reject malformed records.
