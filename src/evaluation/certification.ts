...
export function validateCertificationManifest(manifest: CertificationManifest): void {
  if (manifest.schemaVersion !== 2 || manifest.program.trim().length === 0) throw new Error("Unsupported certification manifest.");
  ...
  for (const task of manifest.tasks) {
    if (!SHA256.test(task.sourceSha256) || !SHA256.test(task.graderSha256)
      ...
      throw new Error(`Task '${task.id}' lacks frozen source/grader/independence digests.`);
    }
  ...
  const tracks = new Set(holdout.map((task) => task.category));
  for (const engine of manifest.engines) {
...
export function createBlindedAssignments(
    ...
    for (let repetition = 1; repetition <= manifest.repetitions; repetition += 1) {
      ...
      for (const [ordinal, engine] of engines.entries()) {
        ...
        privateAssignments.push({
          ...publicAssignment,
          engineId: engine.id,
          privateBindingSha256: bindingSha256(digest, { ...publicAssignment, engineId: engine.id }),
        });
      }
    }
  }
  return {
    publicArtifact: {
      ...
      assignments: shuffled(publicAssignments, rng),
    },
    privateArtifact: {
      schemaVersion: 1,
...
export function validateAssignmentArtifacts(
  ...
  for (const assignment of publicArtifact.assignments) {
    if (Object.hasOwn(assignment as object, "engineId") || Object.hasOwn(assignment as object, "privateBindingSha256")) {
      throw new Error("Public assignment artifact leaks private engine mapping.");
    }
    ...
    const expectedPrivate = bindingSha256(digest, { ...assignment, engineId: privateAssignment.engineId });
...
export function maintainabilityScore(manifest: CertificationManifest, result: BlindRunResult): number {
  ...
  return result.maintainability.adjudication?.score
    ?? mean(result.maintainability.primaryReviews.map((review) => review.score));
}
...
function validateUsage(usage: NormalizedUsageEvidence): void {
  ...
  if (!SHA256.test(usage.evidenceSha256)) throw new Error("Usage evidence digest is malformed.");
