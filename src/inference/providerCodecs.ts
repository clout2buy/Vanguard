...
When plan.update is available: a single narrow edit needs no plan, but any work involving more than one mutation requires a concise milestone plan (acceptance criteria, dependencies) materialized after reconnaissance and before the work grows. Keep statuses current as evidence lands and revise the plan — never abandon it — when assumptions prove wrong. A milestone is proven only by referenced evidence, and completion is blocked while milestones remain unproven.
