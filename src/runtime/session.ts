...
export async function createSessionShell(source: string): Promise<CodingSession> {
  ...
  const session: CodingSession = {
    ...
    materialized: false,
    sourceFingerprint: await fingerprintSource(sourceRoot),
...
export async function materializeSessionWorkspace(session: CodingSession): Promise<CodingSession> {
  ...
  const sourceChanged = session.sourceFingerprint !== undefined
    && await fingerprintSource(session.sourceRoot) !== session.sourceFingerprint;
...
/**
 * A cheap conversation-time change detector. Apply-time correctness never
 * trusts this value: it uses the content-addressed baseline instead.
 */
  ...
  while (queue.length > 0) {
    ...
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      if (details.isSymbolicLink()) {
        entries.push(`${path.relative(root, absolute).replaceAll("\\", "/")}\0link\0${details.mtimeMs}`);
      } else if (details.isDirectory()) {
        if (!SESSION_EXCLUDED_DIRECTORIES.has(entry.name)) queue.push(absolute);
      } else if (details.isFile()) {
        entries.push(`${path.relative(root, absolute).replaceAll("\\", "/")}\0${details.size}\0${details.mtimeMs}`);
      }
    }
  }
  entries.sort();
...
async function writeSessionMetadata(session: CodingSession): Promise<void> {
  await atomicWriteJson(session.metadataFile, {
    id: session.id,
